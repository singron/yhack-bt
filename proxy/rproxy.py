...
class MyReverseProxy(Resource):
    ...
    def __init__(self, host, port, path, engine, reactor=reactor):
        """
    ...
    def getChild(self, path, request):
        ...
        C{path} at the end.
        """
        return MyReverseProxy(
            self.host, self.port, self.path + '/' + urlquote(path, safe=""), self.engine, self.reactor)
    ...
    def render(self, request):
        ...
        """
        print request.uri
        if not check_active(request.uri, self.engine):
